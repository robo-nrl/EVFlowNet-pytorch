...
class upsample_and_predict_flow(nn.Module):
    """
    ...
    def __init__(self, in_channels, out_channels, ksize=3, do_batch_norm=False):
        super(upsample_and_predict_flow, self).__init__()
        self._in_channels = in_channels
        self._out_channels = out_channels
        self._ksize = ksize
        self._do_batch_norm = do_batch_norm
        self._upsample_ch = 32

        self.deconv = general_conv2d(in_channels=self._in_channels,
        ...
        self.predict_flow = general_conv2d(in_channels=32,
                                           out_channels=2,
                                           ksize=1,
                                           strides=1,
                                           padding=0,
    ...
    def forward(self, conv):
        ...
        conv = self.pad(conv)
        deconv = self.deconv(conv)

        flow = self.predict_flow(self.upsample32(conv))# * 256.
        
        return torch.cat([deconv,flow.clone()], dim=1), flow
...
if __name__ == "__main__":
    a = upsample_and_predict_flow(1,4)
